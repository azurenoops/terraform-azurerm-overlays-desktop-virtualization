# Azure Azure Virtual Desktop virtual Machine Overlay Terraform Module

This terraform overlay module is intended for the deployment of Azure virtual machines(Azure Virtual Desktop) with Public IP, proximity placement group, Availability Set, boot diagnostics, data disks, and Network Security Group support. It allows you to use current SSH keys or generate new ones if necessary.
...
## Module Usage to create Azure Virtual Desktop Virtual machine with optional resources
...
module "mod_virtual_machine" {
  ...
  resource_group_name  = azurerm_resource_group.Azure Virtual Desktop-rg.name
  location             = var.location
  environment          = var.environment
  org_name             = var.org_name
  workload_name        = var.workload_name
  virtual_network_name = azurerm_virtual_network.Azure Virtual Desktop-vnet.name
  subnet_name          = azurerm_subnet.Azure Virtual Desktop-snet.name

  # This module supports a variety of pre-configured Azure Virtual Desktop and Windows distributions.
  ...
  existing_network_security_group_name = azurerm_network_security_group.Azure Virtual Desktop-nsg.name
  nsg_inbound_rules = [
    {
      name                   = "ssh"
  ...
  # Attach a managed data disk to a Windows/Azure Virtual Desktop virtual machine. 
 ...
 # (Optional) To save monitoring logs to storage, specify'storage_account_name'.    
  log_analytics_workspace_id = azurerm_log_analytics_workspace.Azure Virtual Desktop-log.id
  ...
  log_analytics_customer_id                  = azurerm_log_analytics_workspace.Azure Virtual Desktop-log.workspace_id
  log_analytics_workspace_primary_shared_key = azurerm_log_analytics_workspace.Azure Virtual Desktop-log.primary_shared_key
